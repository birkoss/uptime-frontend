...
class Endpoint extends Component {
                ...
                if (this.props['apiEndpoint'] === "stats") {
                        res.forEach(stat => {
                            data.push({
                                ...
                                count: stat['response_time__count'],
                                fullDate: stat['grouping'],
                            });
                    });
                } else {
                    for (let key in res) {
    ...
    toolTipContent(abc) {
        if (abc.active) {
            // Fetch the correct stats
            let stat = this.state.stats.filter(stat => stat.date === abc.label);
            if (stat.length !== 1) {
                return null;
            }
            ...
            return (
                <div className="custom-tooltip">
                    <p className="intro">{stat[0].fullDate} ({stat[0].count})</p>
                    {
                        abc.payload.map(payload => {
                            return (
                                <p key={ payload.name } className="label">{`${payload.name} : ${payload.value}`}</p>
                            );
                        })
                    }
                </div>
                    ...
                    { this.props['apiEndpoint'] === 'stats' ?
                        (
                            <LineChart data={this.state.stats} margin={{ top: 5, right: 30, left: 20, bottom: 5, }}>
                                ...
                                <XAxis dataKey="date" />
                                <YAxis />
                                <Tooltip content={ abc => this.toolTipContent(abc) } />
                            ...
                            <BarChart data={this.state.stats} margin={{ top: 20, right: 30, left: 20, bottom: 5, }}>
                                <CartesianGrid strokeDasharray="3 3" />
                                <XAxis dataKey="date" />
                                <YAxis />
                                <Tooltip />
